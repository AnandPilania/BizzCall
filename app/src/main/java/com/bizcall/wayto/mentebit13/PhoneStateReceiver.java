...
public class PhoneStateReceiver extends BroadcastReceiver
{
    ...
    int ch=0;
    String duration1, counselorid, date1,clienturl;
    ...
    {// context=context1;
       ...
       // counselorid=counselorid.replace(" ","");
        editor.putString("RecordedFile1",null);
        editor.commit();
            ...
            telephony.listen(new PhoneStateListener(){
                ...
                public void onCallStateChanged(int state, String incomingNumber)
                {
                   ...
                   Log.d("Number* : ",incomingNumber);
                    if (state==1)
                    {
                        String time1 = new CommonMethods().getTIme();
                        time1 = time1.replaceAll(" ", "");
                        time1=time1.replaceAll(":","_");
                        date1 = new CommonMethods().getDate();
                        ...
                        String rec = path + "/" +phoneNumber + "_" + date1 + "_" + time1 +"_incoming.mp3";
                        Log.d("RecName",rec);
                        ...
                        if(phoneNumber!=null) {
                            try {
                                ...
                                Log.d("ExcPhone", e.toString());
                                Toast.makeText(context, "Exception in start recording xyz " + e.toString(), Toast.LENGTH_SHORT).show();
                            }
                        }
                    ...
                    else if (state==2)
                    {
                     ...
                     /* if (pref.getInt("numOfCalls", 1) == 1)
                        ...
                        if(Intent.ACTION_NEW_OUTGOING_CALL.equals(intent.getAction())){
                            ...
                            time = time.replaceAll(" ", "").replaceAll(":","_");
                            date1 = new CommonMethods().getDate().replaceAll(" ","").replaceAll(":","_");
                            if(phoneNumber.startsWith("+")) {
                                phoneNumber = phoneNumber.substring((phoneNumber.length()) - 10);
                            }
                            phoneNumber = phoneNumber.replaceAll(" ", "");
                            String path = new CommonMethods().getPath();
                            String rec = path + "/" +phoneNumber + "_" + date1 + "_" + time + "_outgoing.mp3";
                           // editor=sp.edit();
                            editor.putString("RecordedFile1",rec);
                            editor.commit();
                            Log.d("RecName",rec+phoneNumber);
                           /* called="Outgoing";
                            phoneNumber = phoneNumber.substring((phoneNumber.length()) - 10);
                            phoneNumber = phoneNumber.replaceAll(" ", "");
                            checkIncomingNumber(phoneNumber,context);*/
                            ...
                            {
                                Log.d("ExcPhone", e.toString());
                                Toast.makeText(context,"Exception in start recording abc "+e.toString(),Toast.LENGTH_SHORT).show();
                            }
                        }
    ...
    public void checkIncomingNumber(String number, final Context context1)
    {
                                       ...
                                       // dialog.dismiss();
                                        if (response.contains("[]"))
                                        {
                                            ...
                                            }catch (Exception e)
                                            {
                                                ...
                                                Toast.makeText(context1,"Exception in call recorder abc",Toast.LENGTH_SHORT).show();
                                            }
        ...
        {
            Log.d("GetAllDetails",e.toString());
