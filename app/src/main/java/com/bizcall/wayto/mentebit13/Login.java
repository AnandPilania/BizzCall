...
public class Login extends AppCompatActivity /*implements GoogleApiClient.ConnectionCallbacks,
    ...
    String callPhoneno, finalCallType, strdateFormated, callDuration, strImei1, strImei2, strdeviceid, callphAccID;
    ...
    String clienturl, deviceId, clientid;
    ...
    String website_url, actname, manufacturer, role;
    String uname, userpass, emailID, emailpass, mobile, statusid;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        try {
            ...
            actname = getIntent().getStringExtra("ActLogin");
                                    ...
                                    try {
                                        ...
                                        if (!response.contains("User not Identified")) {
                                            ...
                                            for (int i = 0; i < jsonArray.length(); i++) {
                                                ...
                                                editor.putString("verifyid", clientid1);
            ...
            if (CheckServer.isServerReachable(Login.this)) {
                ...
                urlRequest.getResponse(new ServerCallback() {
                    ...
                    public void onSuccess(String response) throws JSONException {
                        ...
                        //get DeviceID.
                        deviceId = Settings.Secure.getString(Login.this.getContentResolver(), Settings.Secure.ANDROID_ID);
                        Log.d("deviceidQ", deviceId);
                        ...
                        for (int i = 0; i < jsonArray.length(); i++) {
                            ...
                            strImei1 = jsonObject1.getString("IMEI1");
                            strImei2 = jsonObject1.getString("IMEI2");
                            strdeviceid = jsonObject1.getString("cDeviceID");
                            Log.d("loginimei", role + " / " + strImei1 + " / " + strImei2);
                        }

                        //  final String emailID=jsonObject1.getString("cEmailAdd");
                        ...
                        // final String emailID = response.substring(response.indexOf("[cEmailAdd] =>") + 14, response.indexOf("[cEmailPassword] =>"));
                        // final String emailpass = response.substring(response.indexOf("[cEmailPassword] =>") + 20, response.indexOf("[cMobileNO] =>"));
                        // final String mobile = response.substring(response.indexOf("[cMobileNO] =>") + 14, response.indexOf("[cProfilePhoto] => "));
                        ...
                        if (strdeviceid.equals("0")) {
                            //Log.d("IMEI", strImei1 + " " + strImei2);
                            Log.d("DeviceId", deviceId);
                            int condition = 0;
                            try {
                                if (deviceId == null) {
                                    Toast.makeText(Login.this, "Can't get your Device ID.", Toast.LENGTH_SHORT).show();
                                } else {
                            ...
                            if (condition == 1) {
                                AlertDialog.Builder alertDialogBuilder = new AlertDialog.Builder(Login.this);
                                alertDialogBuilder.setTitle("Your Device ID is:" + deviceId)
                                        ...
                                        .setPositiveButton("Yes", new DialogInterface.OnClickListener() {
                                            public void onClick(DialogInterface dialog, int which) {
                                                insertIMEI();
                                                editor.putString("IMEI1", strImei1);
                                                editor.putString("IMEI2", strImei2);
                        ...
                        else if (strdeviceid.equals(deviceId)/*strImei1.equals(IMEINumber1) || strImei2.equals(IMEINumber1) || strImei1.equals(IMEINumber2) || strImei2.equals(IMEINumber2)*/) {
                            editor.putString("Name", uname);
                            editor.putString("UserPass", userpass);
                            editor.putString("Id", counselorId);
                            editor.putString("EmailId", emailID);
                            ...
                            AlertDialog.Builder alertDialogBuilder = new AlertDialog.Builder(Login.this);
                            alertDialogBuilder.setTitle("Your Device ID is:" + deviceId)
                                    .setMessage("This user is attached with other Device ID.You cannot login on this device using same id")
    ...
    public void insertLoginInfo() {
        try {
            if (CheckServer.isServerReachable(Login.this)) {
                ...
                Log.d("LoginInfoResponse", clienturl + "?clientid=" + clientid + "&caseid=16&CounsellorId=" + counselorId + "&IpAddress1=" + ip);
                urlRequest.getResponse(new ServerCallback() {
                    @Override
                    public void onSuccess(String response) throws JSONException {
                        if (dialog.isShowing()) {
    ...
    public void insertIMEI() {
        try {
            if (CheckServer.isServerReachable(Login.this)) {
                urlRequest = UrlRequest.getObject();
                urlRequest.setContext(getApplicationContext());
                String urlIMEI = clienturl + "?clientid=" + clientid + "&caseid=49&CounselorID=" + counselorId + "&cDeviceID=" + deviceId;

                String checIMEIResponse = checkIMEI(deviceId);
                ...
                if (checIMEIResponse.equals("0")) {
                    ...
                    alertDialogBuilder.setTitle("Device authorization failed")
                            .setMessage("This DeviceID" + deviceId + " is already assigned. Contact admin")
                            // Specifying a listener allows you to take an action before dismissing the dialog.
                            // The dialog is automatically dismissed when a dialog button is clicked.
                            .setPositiveButton("OK", new DialogInterface.OnClickListener() {
                                public void onClick(DialogInterface dialog, int which) {
                                    ...
                                    intent.putExtra("ActLogin", "Login");
                                    startActivity(intent);
                                    dialog.dismiss();
                                }
                            })
    ...
    public String checkIMEI(String strIMEI) {
        try {
            if (CheckServer.isServerReachable(Login.this)) {
                urlRequest = UrlRequest.getObject();
                urlRequest.setContext(getApplicationContext());
                String checkIMEIurl = clienturl + "?clientid=" + clientid + "&caseid=50&cDeviceID=" + strIMEI;
